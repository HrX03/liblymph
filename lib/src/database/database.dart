...
import 'package:libleaf/src/app/directories.dart';
import 'package:libleaf/src/app/providers.dart';
...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> _migrateImagesFromV6() async {
    ...
    final Directories directories = Providers.instance.directories;
    final String defaultImagesDir = directories.imagesDirectory.path;
    ...
    for (final Note note in dbNotes) {
      ...
      notesWithImages[note] =
          savedImages.map((e) => e.getPath(defaultImagesDir)).toList();
      note.images.clear();
    }
    ...
    for (final SavedImage oldImage in oldImages) {
      ...
      for (final noteWithImage in notesWithImages.entries) {
        if (noteWithImage.value.contains(oldImage.getPath(defaultImagesDir))) {
